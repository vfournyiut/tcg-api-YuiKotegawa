...
function createRoom() {
    ...
    if (!deckId || isNaN(deckId)) return alert('Deck ID required (must be a number)');
...
function joinRoom() {
    ...
    if (!roomId || isNaN(roomId) || !deckId || isNaN(deckId)) return alert('Room ID and Deck ID required (must be numbers)');
